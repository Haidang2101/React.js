...
import DetailPost from './b6/DetailPost';
interface Post{
    id: number;
    title: string;
    content: string;
...
export default class BT6 extends Component<{}, Posts> {
    constructor(props: {}) {
        super(props);
        this.state = {
      posts: [
    ...
    return (
        <>
            {this.state.posts.map(item => (
                <DetailPost id={item.id} title={item.title} content={item.content} author={ item.author}></DetailPost>
            ))}
        </>
    )
  }
